        ...
        <!-- <div class="col-md-12"> -->
            <div class="panel panel-default">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group" style="display:none;" id="notifier-party-address">
                    ...
                    <div class="form-group" style="display:none;" id="consignee-party-address">
                ...
                <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
                <div class="col-sm-12">
                    <hr><div class="panel-heading" align="center"><b><h3>Add Port And Country</h3></b></div>
                </div>
                ...
                <!-- <div class="panel-body product_remove"> -->
                                ...
                                <td>
                                    <select class="form-control select2" name="discharge_port_id[]" required>
                                        ...
                                        @foreach($discharge_ports as $discharge_port)
                                            <option value="{{ $discharge_port->name }}">{{ $discharge_port->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <select class="form-control select2" name="final_destination_id[]" required>
                                        ...
                                        @foreach($final_destinations as $final_destination)
                                            <option value="{{ $final_destination->name }}">{{ $final_destination->name }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                    ...
                    <div class="panel-heading" align="left"><button type="button" class="btn btn-warning add-port-row">Add More Port</button></div>
                <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
                <div class="col-sm-12">
                    <hr>
                    <br><div class="panel-heading" align="center"><b><h3>Add Shipment Details</h3></b></div><br>
                </div>
                <hr>
                ...
                <div class="panel-body remove">
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="col-sm-4 control-label">Shipment Code</label>
                            <div class="col-sm-8">
                                <section class="form-control select2" id="shipment-code" name="shipment_code[]">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="col-sm-4 control-label">Quotation Freight</label>
                            <div class="col-sm-4">
                    ...
                    <!-- <div class="col-md-4">
                        <div class="panel-heading"><button type="button" class="btn btn-danger remove_shipment">Remove Shipment</button></div>
                    </div> -->
                    </div>
                    </div>
                    ...
                    <!-- ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ -->
                <div class="col-sm-12">
                    <hr><div class="panel-heading" align="center"><b><h3>Add Product</h3></b></div>
                </div>
                <!-- <div class="product_detail"> -->
                <!-- <div class="panel-body product_remove"> -->
                    ...
                    <div class="panel-heading" align="left"><button type="button" class="btn btn-warning add-product-row add_more_product">Add More Product</button></div>

                    <div class="panel-heading" align="right"><button class="btn btn-primary"><i class=""></i> Save</button></div>
                </form>
                </div>
            </div>
        <!-- </div> -->
...
$(document).ready(function() {
    ...
    $('.add_more_product').on('click',function(){
        $('.product_row').append('<tr class="product-cal"><td><section class="form-control select2" id="shipment-code" name="shipment_code"><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option><option value="E">E</option><option value="F">F</option><option value="G">G</option><option value="H">H</option><option value="I">I</option><option value="J">J</option></section></td><td><select class="form-control select2 discharge_port_auto_fillup_opt" name=""><option></option></select></td><td><select class="form-control select2" name=""> @foreach($products as $product) <option value="{{ $product->id }}">{{ $product->name }}</option> @endforeach </select></td><td><select class="form-control select2" name=""> @foreach($packages as $package) <option value="{{ $package->id }}">{{ $package->name }}</option> @endforeach </select></td><td><input type="text" name="" class="form-control"></td><td><input type="text" name="qty[]" class="form-control"></td><td><input type="text" name="rate[]" class="form-control"></td><td><input type="text" name="amount[]" class="form-control" readonly></td><td><button type="button" class="btn btn-danger btn-sm product-remove-row"><i class="fa fa-window-close-o"></i></button></td></tr>');
        $('.select2').select2();
    ...
    $('.add-port-row').on('click',function(){
        $('.port_row').append('<tr><td><select class="form-control select2" name="loading_port_id[]" required><option value="">Select Any One</option> @foreach($ports as $port) <option value="{{ $port->name }}">{{ $port->name }}</option> @endforeach </select></td><td><select class="form-control select2" name="discharge_port_id[]" required><option value="">Select Any One</option> @foreach($discharge_ports as $discharge_port) <option value="{{ $discharge_port->name }}">{{ $discharge_port->name }}</option> @endforeach </select></td><td><select class="form-control select2" name="final_destination_id[]" required><option value="">Select Any One</option> @foreach($final_destinations as $final_destination) <option value="{{ $final_destination->name }}">{{ $final_destination->name }}</option> @endforeach </select></td><td><select class="form-control select2" name="destination_country_id[]" required><option value="">Select Any One</option> @foreach($countries as $countrie) <option value="{{ $countrie->name }}">{{ $countrie->name }}</option> @endforeach </select> </td><td><button type="button" class="btn btn-danger btn-sm port-remove-row"><i class="fa fa-window-close-o"></i></button></td></tr>');
        $('.select2').select2();
    });
    ...
    function discharge_port_auto_fillup(){
        var options = "";
            $.each($('select[name="loading_port_id[]"]'), function(i, v) {
                options += "<option value='" + v + "' selected='selected'>" + $('select[name="loading_port_id[]"]').eq(i).val() +","+ 
                    $('select[name="discharge_port_id[]"]').eq(i).val() +"," + $('select[name="final_destination_id[]"]').eq(i).val() +"," + $('select[name="destination_country_id[]"]').eq(i).val() +"," + 
                    "</option>";
                });
