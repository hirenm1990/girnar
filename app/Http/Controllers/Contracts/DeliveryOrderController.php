...
class DeliveryOrderController extends Controller
{
    ...
    public function update( Request $request, $shipment_id )
    {
    	...
    	if(!empty($do_id)) {
    		...
    		( $input['booking_date'] !="" ) ? $do->booking_date = date('Y-m-d',strtotime($input['booking_date'])) :'NULL';	
    		( $input['booking_expiry_date'] !="" ) ? $do->booking_expiry_date = date('Y-m-d',strtotime($input['booking_expiry_date'])) :"NULL";	
    		( $input['document_cutoff'] !="" ) ? $do->document_cutoff = date('Y-m-d',strtotime($input['document_cutoff'])) :"";
    		( $input['si_cutoff'] !="" ) ? $do->si_cutoff = date('Y-m-d',strtotime($input['si_cutoff'])) :"";
    		( $input['eta_origin'] !="" ) ? $do->eta_origin = date('Y-m-d',strtotime($input['eta_origin'])) :"";
    		( $input['etd_origin'] !="" ) ? $do->etd_origin = date('Y-m-d',strtotime($input['etd_origin'])) :"";
    		( $input['eta_destination'] !="" ) ? $do->eta_destination = date('Y-m-d',strtotime($input['eta_destination'])) :"";
    		( $input['eta_date'] !="" ) ? $do->eta_date = date('Y-m-d',strtotime($input['eta_date'])) :"";
    		...
    		( $input['change_eta_destination_port'] !="" ) ? $do->change_eta_destination_port = date('Y-m-d',strtotime($input['change_eta_destination_port'])) :"";
    		$do->change_total_transit_time = $input['change_total_transit_time'];
    		( $input['stuffing_from'] !="" ) ? $do->stuffing_from = date('Y-m-d',strtotime($input['stuffing_from'])) :"";
    		( $input['stuffing_to'] !="" ) ? $do->stuffing_to = date('Y-m-d',strtotime($input['stuffing_to'])) :"";
    		$do->save();
